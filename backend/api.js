...
app.post("/api/reservas-pagos", async (req, res) => {
  const {
    id_cliente,
    id_paquete,
    fecha_reserva,
  ...
  const client = await pool.connect(); // Usar transacciones para asegurar atomicidad

  try {
    // Validar los datos
    if (!id_cliente || !id_paquete || !fecha_reserva || !hora_inicio || !hora_fin || !monto || !metodo_pago) {
      return res.status(400).json({ error: "Todos los campos son obligatorios." });
    }
    ...
    const pagoQuery = `
      INSERT INTO public.pagos (monto, metodo_pago, fecha_pago, id_reserva, id_estado)
      VALUES ($1, $2, $3, $4, (SELECT id_estado FROM estado WHERE nombre = 'Pagado'))
      RETURNING *;
    `;

    const pagoValues = [monto, metodo_pago, fecha_pago, reserva.id_reserva];
    ...
    console.error("Error al procesar la reserva y el pago:", error.message);
    res.status(500).json({ error: "Error al procesar la reserva y el pago." });
  } finally {
    client.release();
  }
});
